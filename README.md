...
| **Git**        | Version control                     |
| **Nginx**      | Container image used by the Pod     |
| **Terminal**   | Creating and managing project files |
...
| `metadata.name`    | Names the Pod                        |
| `spec`             | Defines the desired state            |
| `containers`       | Defines the Pod's containers         |
| `containers.name`  | Names the container                  |
...
If a Kubernetes cluster is available, the manifest can be applied with:

```bash
kubectl apply -f pod.yaml
```

The Pod can then be checked with:
...
These commands are included as part of the next stage of working with the manifest and require a configured Kubernetes environment.
